...
- 算法
    ...
    ## <a id="lb">链表</a>
    链表与数组相比较，他不需要连续的内存空间，他通过指针将不连续的内存块串联起来。
    - 单链表
    我们将内存块成为节点，在单链表中每个链表的节点除了存储数据，还需要存储下个结点的地址，我们将记录下个结点地址的指针叫做后续指针next.
    单链表有两个结点是特殊的，头结点和尾结点，头结点记录链表的基地址。尾节点指针指向空地址NULL。   
    针对链表的插入和删除，时间复杂度为O(1)
    - 循环链表   
    循环链表是一种特殊的单链表，他的尾结点指向链表的头结点，首尾相连。   
    和单链表相比，循环链表的优势是从链尾到链头很方便，当处理的数据具有环形特点时，就可以考虑采用循环链表
    - 双向链表
    单链表只有一个后继指针指向下一个结点，而双向结点还存一个前驱指针prev指向上一个结点.   
    存储同样数据的话 双向链表要比单链表占用更多的空间   
    双向链表的优势在哪尼？   
    对于删除指定值的操作：   
        - 单链表/双向链表： 查找某个给定值需要从头遍历O(n),删除操作O(1)
    删除给定指针执行的节点操作：   
        - 单链表：删除节点需要知道其前驱节点，从头遍历，知道知道该节点的前驱节点O(n)
        - 双向链表：改结点已保存前驱节点，不需要遍历 O(1)
    


    ## <a id="mppx">冒泡排序</a>
    相邻元素进行比较，位置互换。一次冒泡至少一个元素移动到相应的位置上。
    ```
        function test(arr) {
